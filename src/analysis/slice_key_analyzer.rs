...
pub enum SliceKeyAnalysisResult {
    ...
    /// Use the length of the s;oce as hash code, instead of hashing the slices
    Length,
}
...
fn analyze_lengths<T>(unique_slices: &[&[T]]) -> SliceKeyAnalysisResult {
    const MAX_IDENTICAL_LENGTHS: usize = 3;
    const MAX_SLICES: usize = 255;

    if unique_slices.len() > MAX_SLICES {
...
fn analyze_subslices<T: Hash, BH: BuildHasher>(
    unique_slices: &[&[T]],
    bh: &BH,
) -> SliceKeyAnalysisResult {
    ...
    // tolerate a certain amount of duplicate substrings
    let acceptable_duplicates =
        ((unique_slices.len() as f64) * ACCEPTABLE_DUPLICATE_PERCENT) as usize;

    // this set is reused for each call to is_sufficiently_unique
    let mut set = HashSet::with_capacity(unique_slices.len());
    ...
    // could not find a substring that was good enough.
    SliceKeyAnalysisResult::Normal
}

fn is_sufficiently_unique<T: Hash, BH: BuildHasher>(
    unique_slices: &[&[T]],
    subslice_index: usize,
    subslice_len: usize,
    left_justified: bool,
...
mod tests {
    ...
    use super::{analyze_slice_keys, SliceKeyAnalysisResult};

    struct AnalysisTestCase<'a> {
        strings: &'a [&'a str],
        result: SliceKeyAnalysisResult,
    }
    ...
    fn analyze_string_keys_test() {
        const ANALYSIS_TEST_CASES: [AnalysisTestCase; 9] = [
            AnalysisTestCase {
                ...
                result: SliceKeyAnalysisResult::LeftHandSubslice {
                    subslice_index: 1,
                    subslice_len: 1,
                },
            },
            AnalysisTestCase {
                strings: &["A00", "B00", "C00", "D00"],
                result: SliceKeyAnalysisResult::LeftHandSubslice {
                    subslice_index: 0,
                    subslice_len: 1,
                },
            ...
            AnalysisTestCase {
                strings: &["A", "B", "C", "D", "E2", ""],
                result: SliceKeyAnalysisResult::Normal,
            },
            AnalysisTestCase {
                strings: &["XA", "XB", "XC", "XD", "XE2"],
                result: SliceKeyAnalysisResult::LeftHandSubslice {
                    subslice_index: 1,
                    subslice_len: 1,
                },
            ...
            AnalysisTestCase {
                strings: &["XXA", "XXB", "XXC", "XXD", "XXX", "XXXE"],
                result: SliceKeyAnalysisResult::RightHandSubslice {
                    subslice_index: 0,
                    subslice_len: 1,
                },
            },
            AnalysisTestCase {
                strings: &["ABC", "DEFG", "HIJKL", "MNOPQR", "STUVWX", "YZ"],
                result: SliceKeyAnalysisResult::Length,
            },
            AnalysisTestCase {
                strings: &[
            ...
            AnalysisTestCase {
                strings: &["AAA", "1AA", "A1A", "AA1", "BBB", "1BB", "B1B", "BB1"],
                result: SliceKeyAnalysisResult::Normal,
            },
        ];
        ...
        for (count, case) in ANALYSIS_TEST_CASES.into_iter().enumerate() {
            ...
            let keys: Vec<&[u8]> = case.strings.iter().map(|x| x.as_bytes()).collect();
            assert_eq!(
                case.result,
                analyze_slice_keys(keys.as_slice(), &RandomState::new())
            );
        }
    }
}
